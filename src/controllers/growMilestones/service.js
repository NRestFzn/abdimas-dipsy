...
const { GrowMilestones } = models
class growMilestonesService {
  constructor() {}
  ...
  static async findAll(req) {
    ...
    const data = await GrowMilestones.findAll({
      ...restQuery,
    })

    const total = await GrowMilestones.count({
  ...
  static async findById(id) {
    const data = await GrowMilestones.findOne({ where: { id } })
  ...
  static async create(formData, transaction) {
    ...
    const data = await GrowMilestones.create(value, { transaction })
...
module.exports = growMilestonesService
